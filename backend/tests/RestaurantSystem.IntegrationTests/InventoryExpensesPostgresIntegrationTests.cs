...
public sealed class InventoryExpensesPostgresIntegrationTests(PostgresFixture postgres)
{
    ...
    public async Task Inventory_and_expense_contracts_use_real_postgresql()
    {
        ...
        var expense = new { expenseCategoryId = categoryId, amount = 12.5m, cashSource = "PETTY_CASH", description = " supplies ", expenseDate = DateOnly.FromDateTime(TimeZoneInfo.ConvertTimeBySystemTimeZoneId(DateTimeOffset.UtcNow, "America/La_Paz").DateTime).ToString("yyyy-MM-dd") };
