...
builder.Services.AddAuthorization(options =>
{
    ...
    options.AddPolicy(PolicyNames.ExpenseCategoryRead, p => p.RequireRole(RoleNames.Administrator, RoleNames.Manager, RoleNames.Accountant));
...
attendance.MapPost("/me/check-in", async (ClaimsPrincipal principal, IAttendanceService service, CancellationToken ct) => { var result = await service.CheckInSelfAsync(principal.FindFirstValue(ClaimTypes.NameIdentifier)!, ct); return result.Error is null ? Results.Created("/api/v1/attendance/me", result.Value) : AttendanceError(result.Error); }).RequireAuthorization(PolicyNames.AttendanceSelf).Produces<PersonalAttendanceRecordDto>(201).ProducesProblem(400).ProducesProblem(401).ProducesProblem(404).ProducesProblem(409);
    attendance.MapPost("/me/check-out", async (ClaimsPrincipal principal, IAttendanceService service, CancellationToken ct) => { var result = await service.CheckOutSelfAsync(principal.FindFirstValue(ClaimTypes.NameIdentifier)!, ct); return result.Error is null ? Results.Ok(result.Value) : AttendanceError(result.Error); }).RequireAuthorization(PolicyNames.AttendanceSelf).Produces<PersonalAttendanceRecordDto>(200).ProducesProblem(400).ProducesProblem(401).ProducesProblem(404).ProducesProblem(409);
    attendance.MapGet("/me/current", async (ClaimsPrincipal principal, IAttendanceService service, CancellationToken ct) => { var result = await service.CurrentAsync(principal.FindFirstValue(ClaimTypes.NameIdentifier)!, ct); return result.Error is null ? Results.Ok(result.Value) : AttendanceError(result.Error); }).RequireAuthorization(PolicyNames.AttendanceSelf).Produces<AttendanceCurrentResponse>(200).ProducesProblem(401).ProducesProblem(404);
