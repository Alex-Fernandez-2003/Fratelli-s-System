...
public sealed class OperationsService(ApplicationDbContext db, IInventoryWriter inventory, IBusinessClock clock) : IOperationsService
{
    ...
    public async Task<(SaleDto? Value, string? Error, IReadOnlyList<ProductionRequirementDto>? Shortages)> ConfirmSaleAsync(ConfirmSaleRequest request, string actor, CancellationToken ct = default)
    {
        ...
        var ids = order.Items.Select(x => x.ProductId).ToArray(); var products = await db.Products.Where(x => ids.Contains(x.Id)).ToDictionaryAsync(x => x.Id, ct); var balances = await db.InventoryBalances.Where(x => ids.Contains(x.ProductId)).ToDictionaryAsync(x => x.ProductId, x => x.Quantity, ct);
        var precheck = order.Items.GroupBy(x => x.ProductId).Select(x => new ProductionRequirementDto(x.Key, products[x.Key].Name, x.Sum(y => y.Quantity), balances.GetValueOrDefault(x.Key), Math.Max(0, x.Sum(y => y.Quantity) - balances.GetValueOrDefault(x.Key)), products[x.Key].InventoryUnitId)).Where(x => x.ShortageQuantity > 0).ToArray(); if (precheck.Length > 0 && !request.AcknowledgeStockShortage) return (null, "SALE_STOCK_CONFIRMATION_REQUIRED", precheck);
        ...
        var batch = await inventory.WriteBatchAsync(order.Items.GroupBy(x => x.ProductId).Select(x => new InventoryWriteCommand(x.Key, InventoryMovementType.SALE, -x.Sum(y => y.Quantity), "Sale", InventoryReferenceType.SALE, sale.Id, actor)).ToArray(), request.AcknowledgeStockShortage, ct);
        if (batch.Error is not null) { var shortages = batch.Error == "STOCK_INSUFFICIENT" && batch.Value is not null ? batch.Value.Shortages.Select(x => new ProductionRequirementDto(x.ProductId, products[x.ProductId].Name, x.Required, x.Current, Math.Max(0, x.Required - x.Current), products[x.ProductId].InventoryUnitId)).ToArray() : precheck; return (null, "SALE_STOCK_CONFIRMATION_REQUIRED", shortages); }
        await tx.CommitAsync(ct); return (new(sale.Id, sale.OrderId, sale.ShiftId, sale.Subtotal, sale.Total, sale.Items.Select(x => new SaleItemDto(x.ProductId, x.Quantity, x.UnitPrice, x.LineTotal)).ToArray()), null, null);
    }
