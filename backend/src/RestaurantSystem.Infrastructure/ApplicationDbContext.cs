...
public sealed class ApplicationDbContext(DbContextOptions<ApplicationDbContext> options) : IdentityDbContext<IdentityUser>(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        {
            entity.ToTable("Employees", "public"); entity.HasKey(x => x.Id); entity.Property(x => x.UserId).IsRequired(); entity.Property(x => x.FullName).IsRequired(); entity.Property(x => x.HourlyRate).HasPrecision(12, 2);
            ...
            builder.Entity<Supplier>(entity =>
        {
            ...
            {
                e.ToTable("customers", "public"); e.HasKey(x => x.Id);
                e.Property(x => x.Name).IsRequired().HasMaxLength(200);
                e.Property(x => x.Ci).IsRequired().HasMaxLength(50);
                e.Property(x => x.Nit).HasMaxLength(50);
                ...
                e.Property(x => x.CreatedByUserId).IsRequired(); e.Property(x => x.UpdatedByUserId).IsRequired();
                e.HasIndex(x => x.Ci).IsUnique().HasDatabaseName("UX_customers_ci");
                e.HasIndex(x => x.Nit).IsUnique().HasFilter("\"Nit\" IS NOT NULL").HasDatabaseName("UX_customers_nit_not_null");
            });
            ...
            builder.Entity<Production>(e => { e.ToTable("productions","public"); e.HasKey(x=>x.Id); e.Property(x=>x.BatchCode).IsRequired().HasMaxLength(40); e.Property(x=>x.Status).HasConversion<string>().IsRequired().HasMaxLength(20); e.HasIndex(x=>x.BatchCode).IsUnique().HasDatabaseName("UX_productions_batch_code"); e.Property(x=>x.QuantityProduced).HasPrecision(14,4); e.HasCheckConstraint("CK_productions_quantity","\"QuantityProduced\" > 0"); e.HasCheckConstraint("CK_productions_status","\"Status\" IN ('COMPLETED')"); e.HasIndex(x=>new{x.ProductId,x.ProducedAt}); e.HasOne<Product>().WithMany().HasForeignKey(x=>x.ProductId).OnDelete(DeleteBehavior.Restrict); });
            ...
            builder.Entity<CashSession>(e => { e.ToTable("cash_sessions","public"); e.HasKey(x=>x.Id); e.Property(x=>x.OpeningAmount).HasPrecision(12,2); e.Property(x=>x.PettyCashOpeningAmount).HasPrecision(12,2); e.Property(x=>x.CashRemovedAmount).HasPrecision(12,2); e.Property(x=>x.CashAmountCarriedForward).HasPrecision(12,2); e.HasIndex(x=>x.BusinessDate).IsUnique(); }); builder.Entity<CashClosing>(e=>{e.ToTable("cash_closings","public");e.HasKey(x=>x.Id);e.Property(x=>x.OpeningAmount).HasPrecision(12,2);e.Property(x=>x.PettyCashOpeningAmount).HasPrecision(12,2);e.Property(x=>x.CashRemovedAmount).HasPrecision(12,2);e.Property(x=>x.SalesTotal).HasPrecision(12,2);e.Property(x=>x.CashSalesTotal).HasPrecision(12,2);e.Property(x=>x.QrSalesTotal).HasPrecision(12,2);e.Property(x=>x.ExternalSalesTotal).HasPrecision(12,2);e.Property(x=>x.DirectSalesTotal).HasPrecision(12,2);e.Property(x=>x.PedidosYaSalesTotal).HasPrecision(12,2);e.Property(x=>x.CashDrawerExpensesTotal).HasPrecision(12,2);e.Property(x=>x.PettyCashExpensesTotal).HasPrecision(12,2);e.Property(x=>x.ExpensesTotal).HasPrecision(12,2);e.Property(x=>x.ExpectedCash).HasPrecision(12,2);e.Property(x=>x.DeclaredCash).HasPrecision(12,2);e.Property(x=>x.Difference).HasPrecision(12,2);e.Property(x=>x.Observation).HasMaxLength(1000);e.Property(x=>x.ClosedByUserId).IsRequired();e.Property(x=>x.ClosedAt).HasColumnType("timestamp with time zone");e.HasIndex(x=>x.CashSessionId).IsUnique();e.HasIndex(x=>x.BusinessDate);e.HasOne<CashSession>().WithMany().HasForeignKey(x=>x.CashSessionId).OnDelete(DeleteBehavior.Restrict);e.HasOne<IdentityUser>().WithMany().HasForeignKey(x=>x.ClosedByUserId).HasPrincipalKey(x=>x.Id).OnDelete(DeleteBehavior.Restrict);}); builder.Entity<Shift>(e=> { e.ToTable("shifts","public"); e.HasKey(x=>x.Id); e.Property(x=>x.Type).HasConversion<string>(); e.Property(x=>x.Status).HasConversion<string>(); e.HasIndex(x=>new{x.CashSessionId,x.Type}).IsUnique(); e.HasOne<CashSession>().WithMany(x=>x.Shifts).HasForeignKey(x=>x.CashSessionId).OnDelete(DeleteBehavior.Restrict); }); builder.Entity<WorkSchedule>(e=>{e.ToTable("work_schedules","public");e.HasKey(x=>x.Id);e.Property(x=>x.ShiftType).HasConversion<string>().IsRequired();e.Property(x=>x.PlannedStart).HasColumnType("time without time zone");e.Property(x=>x.PlannedEnd).HasColumnType("time without time zone");e.HasIndex(x=>x.ShiftType).IsUnique();e.HasCheckConstraint("CK_work_schedules_time_range", "\"PlannedStart\" <> \"PlannedEnd\"");e.HasCheckConstraint("CK_work_schedules_late_tolerance", "\"LateToleranceMinutes\" >= 0");}); builder.Entity<ShiftAssignment>(e=>{e.ToTable("shift_assignments","public");e.HasKey(x=>x.Id);e.Property(x=>x.EffectivePlannedStart).HasColumnType("time without time zone");e.Property(x=>x.EffectivePlannedEnd).HasColumnType("time without time zone");e.HasIndex(x=>new{x.ShiftId,x.EmployeeId}).IsUnique();e.HasOne<Shift>().WithMany(x=>x.Assignments).HasForeignKey(x=>x.ShiftId).OnDelete(DeleteBehavior.Restrict);e.HasOne<Employee>().WithMany().HasForeignKey(x=>x.EmployeeId).OnDelete(DeleteBehavior.Restrict);});
            builder.Entity<Sale>(e=>{e.ToTable("sales","public");e.HasKey(x=>x.Id);e.Property(x=>x.CustomerNameSnapshot).HasMaxLength(200);e.Property(x=>x.CustomerCiSnapshot).HasMaxLength(50);e.Property(x=>x.CustomerNitSnapshot).HasMaxLength(50);e.Property(x=>x.SalesChannel).HasConversion<string>();e.Property(x=>x.PaymentMethod).HasConversion<string>();e.Property(x=>x.Subtotal).HasPrecision(12,2);e.Property(x=>x.Total).HasPrecision(12,2);e.HasIndex(x=>x.OrderId).IsUnique();e.HasIndex(x=>x.CustomerId);e.HasOne<Shift>().WithMany().HasForeignKey(x=>x.ShiftId).OnDelete(DeleteBehavior.Restrict);e.HasOne<Customer>().WithMany().HasForeignKey(x=>x.CustomerId).OnDelete(DeleteBehavior.Restrict);});builder.Entity<SaleItem>(e=>{e.ToTable("sale_items","public");e.HasKey(x=>x.Id);e.Property(x=>x.Quantity).HasPrecision(14,4);e.Property(x=>x.UnitPrice).HasPrecision(12,2);e.Property(x=>x.LineTotal).HasPrecision(12,2);e.HasIndex(x=>new{x.SaleId,x.OrderItemId}).IsUnique();e.HasOne<Sale>().WithMany(x=>x.Items).HasForeignKey(x=>x.SaleId).OnDelete(DeleteBehavior.Restrict);});
