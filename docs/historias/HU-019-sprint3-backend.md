...
Backend completo para HU-019. Frontend productivo fuera de alcance de este change.
...
- `GET /api/v1/purchases?page=&pageSize=&status=` → `PagedResponse<PurchaseDto>` (compatibilidad Sprint2)
...
- `curl http://localhost:5057/openapi/v1.json` → 200 (289K) con paths HU-019 presentes
...
| Archivo | Propósito |
| --- | --- |
| `backend/src/RestaurantSystem.Application/Operations/OperationalContracts.cs` | PurchaseHistoryDto, PurchaseDetailDto + PurchaseArea derivado |
| `backend/src/RestaurantSystem.Infrastructure/Operations/OperationsService.cs` | PurchaseHistoryAsync, PurchaseDetailAsync, KitchenPurchases scope |
| `backend/src/RestaurantSystem.Api/OperationsEndpoints.cs` | GET /api/v1/purchases, GET /api/v1/purchases/history, GET /purchases/history/{id} |
| `backend/src/RestaurantSystem.Api/Program.cs` | Policy PurchaseHistory |
| `backend/tests/RestaurantSystem.IntegrationTests/OperationsAuthorizationMatrixPostgresIntegrationTests.cs` | COCINA KITCHEN scope |
...
| Archivo | Propósito |
| --- | --- |
| `docs/historias/HU-019-sprint3-backend.md` | Esta HU (backend completo, endpoints + manifest) |
...
`HU_019_BACKEND_COMPLETE: YES` — `READY_FOR_SPRINT_3_FRONTEND: YES` (frontend Sprint 3 pendiente en change separado)
