...
Backend completo para HU-021. Frontend productivo fuera de alcance de este change.
...
- `curl http://localhost:5057/openapi/v1.json` → 200 (289K) con paths HU-021 presentes
...
| Archivo | Propósito |
| --- | --- |
| `backend/src/RestaurantSystem.Application/Expenses/ExpenseContracts.cs` | ExpenseHistoryDto, ExpenseHistoryPage |
| `backend/src/RestaurantSystem.Infrastructure/Expenses/ExpenseService.cs` | HistoryAsync con full-filter aggregates (cashDrawer/petty/total) |
| `backend/src/RestaurantSystem.Api/Program.cs` | Policy ExpenseHistory |
| `backend/src/RestaurantSystem.Infrastructure/Operations/OperationsService.cs` | integración shift/cashSession para BusinessDate |
...
| Archivo | Propósito |
| --- | --- |
| `docs/historias/HU-021-sprint3-backend.md` | Esta HU (backend completo, endpoints + manifest) |
...
`HU_021_BACKEND_COMPLETE: YES` — `READY_FOR_SPRINT_3_FRONTEND: YES` (frontend Sprint 3 pendiente en change separado)
