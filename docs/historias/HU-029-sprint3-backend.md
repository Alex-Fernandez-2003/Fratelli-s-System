...
Backend completo para HU-029. Frontend productivo fuera de alcance de este change.

## Reglas implementadas

Ver `docs/openspec/changes/archive/2026-08-31-implement-sprint-3-complete-backend/spec.md` y `design.md` para reglas normativas congeladas de HU-029.
...
- `GET /api/v1/reports/sales?from=&to=` → `SalesReportDto{salesCount,totalAmount,cash/qr/external/direct/pedidosYa, series: SalesReportSeriesDto[]}` reusa scope HU-015
...
- `curl http://localhost:5057/openapi/v1.json` → 200 (289K) con paths HU-029 presentes
...
| Archivo | Propósito |
| --- | --- |
| `backend/src/RestaurantSystem.Application/Operations/OperationalContracts.cs` | SalesReportDto, SalesReportSeriesDto |
| `backend/src/RestaurantSystem.Infrastructure/Operations/OperationsService.cs` | SalesReportAsync (reusa ISalesAuthorizationScope, aggregates + BusinessDate series) |
| `backend/src/RestaurantSystem.Api/OperationsEndpoints.cs` | GET /api/v1/reports/sales (SalesHistory) |
...
| Archivo | Propósito |
| --- | --- |
| `docs/historias/HU-029-sprint3-backend.md` | Esta HU (backend completo, endpoints + manifest) |
...
`HU_029_BACKEND_COMPLETE: YES` — `READY_FOR_SPRINT_3_FRONTEND: YES` (frontend Sprint 3 pendiente en change separado)
