...
Backend completo para HU-028. Frontend productivo fuera de alcance de este change.

## Reglas implementadas

Ver `docs/openspec/changes/archive/2026-08-31-implement-sprint-3-complete-backend/spec.md` y `design.md` para reglas normativas congeladas de HU-028.
...
- `GET /api/v1/cash/closings/{id}` → `CashClosingDto` solo snapshots inmutables
...
- `curl http://localhost:5057/openapi/v1.json` → 200 (289K) con paths HU-028 presentes
...
| Archivo | Propósito |
| --- | --- |
| `backend/src/RestaurantSystem.Infrastructure/Operations/OperationsService.cs` | CashClosingsAsync, CashClosingAsync (solo snapshots) |
| `backend/src/RestaurantSystem.Api/OperationsEndpoints.cs` | GET /api/v1/cash/closings, GET /api/v1/cash/closings/{id} (CashHistory) |
...
| Archivo | Propósito |
| --- | --- |
| `docs/historias/HU-028-sprint3-backend.md` | Esta HU (backend completo, endpoints + manifest) |
...
`HU_028_BACKEND_COMPLETE: YES` — `READY_FOR_SPRINT_3_FRONTEND: YES` (frontend Sprint 3 pendiente en change separado)
